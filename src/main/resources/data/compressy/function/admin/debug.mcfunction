...
# Show config values
execute store result score #max_level cb.temp run data get storage compressy:config max_compression_level
tellraw @s [{"text":"Config Max Level: ","color":"gray"},{"score":{"name":"#max_level","objective":"cb.temp"},"color":"white"}]

execute store result score #debug cb.temp if data storage compressy:config {debug_mode:true}
