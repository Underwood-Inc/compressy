...
# Get the item ID
data modify storage compressy:temp block_id set from entity @s Inventory[{Slot:-106b}].id
...
# Store for macro
execute store result storage compressy:temp used_count int 1 run scoreboard players get #used cb.temp
...
function compressy:wand/do_compress_initial with storage compressy:temp
