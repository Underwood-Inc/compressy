...
# ========================================
# Compresses already-Compressy to next level
...
# Need 9 to compress
execute unless score #count cb.temp matches 9.. run return run tellraw @s [{"text":"[Compressy] ","color":"gold"},{"text":"Need 9 Compressy to compress further!","color":"red"}]
...
execute store result storage compressy:temp give_count int 1 run scoreboard players get #compress_count cb.temp

# Execute
function compressy:wand/do_compress_higher with storage compressy:temp
