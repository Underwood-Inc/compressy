...
execute unless data entity @s Inventory[{Slot:-106b}] run return run tellraw @s [{"text":"[Compressed Blocks] ","color":"gold"},{"text":"Put blocks in your offhand to compress!","color":"yellow"}]
...
execute unless score #count cb.temp matches 9.. run return run tellraw @s [{"text":"[Compressed Blocks] ","color":"gold"},{"text":"Need at least 9 blocks in offhand!","color":"red"}]
