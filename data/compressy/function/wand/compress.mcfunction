...
data modify storage compressy:temp offhand_item set from entity @s Inventory[{Slot:-106b}]

# Check if it's already compressed
execute if data entity @s Inventory[{Slot:-106b}].components."minecraft:custom_data".compressed_level run function compressy:wand/compress_higher
execute unless data entity @s Inventory[{Slot:-106b}].components."minecraft:custom_data".compressed_level run function compressy:wand/compress_initial
