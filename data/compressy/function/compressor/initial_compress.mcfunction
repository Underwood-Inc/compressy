...
data modify storage compressy:temp block_id set from entity @s SelectedItem.id
...
execute store result storage compressy:temp clear_count int 1 run scoreboard players get #used cb.temp
...
data modify storage compressy:temp item_to_clear set from entity @s SelectedItem.id

# Execute the item giving with macro
function compressy:compressor/give_compressed with storage compressy:temp
