...
execute store result storage compressy:temp hand_count int 1 run clear @s * 0
...
data modify storage compressy:temp held_item set from entity @s Inventory[{Slot:0b}]

# Actually process compression based on what's being held
# This runs a series of checks to handle the compression
function compressy:compressor/process_held_item
